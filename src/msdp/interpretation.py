...
def artifact_confidence(components,sources,weights=None):
    ...
    if not used: return {"score":None,"label":"Unavailable","components":components,"component_sources":sources,"missing_components":missing,"used_components":[]}
    denom=sum(weights[k] for k in used); uncertainty=sum(weights[k]*v for k,v in used.items())/denom; score=float(100*(1-uncertainty)); label="High" if score>=70 else "Medium" if score>=40 else "Low"
